...
const settings = read('src/pages/Dashboard/Settings/index.tsx')
const feedback = read('src/feedback.ts')
const feedbackPage = read('src/pages/Dashboard/Feedback/index.tsx')
const menu = read('src/navigation/Menu.tsx')
...
assertIncludes('flattened settings content', settings, '<AdditionalSettings />')
assertIncludes('legacy advanced settings redirect', dashboard, "path='settings/advanced' element={<Navigate to='/dashboard/settings' replace />}")
assertIncludes('Usercom feedback endpoint', feedback, 'https://usercom.babbage.systems/submit')
assertIncludes('Usercom feedback source', feedback, "source: 'user-wallet'")
assertIncludes('feedback omits credentials', feedback, "credentials: 'omit'")
assertIncludes('feedback omits referrer', feedback, "referrerPolicy: 'no-referrer'")
assertIncludes('feedback secret warning', feedbackPage, 'Do not include passwords, recovery phrases, private keys')
assertIncludes('feedback navigation entry', menu, "testId: 'wallet-nav-feedback'")
assertIncludes('feedback route', dashboard, "path='feedback' element={<Feedback />}")
