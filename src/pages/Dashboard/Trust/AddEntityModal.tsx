...
type AddEntityModalProps = {
  ...
  setTrustedEntities: Dispatch<SetStateAction<Certifier[]>>
}
...
const AddEntityModal = ({
  ...
  const handleDirectSubmit = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    try {
      setLoading(true)
      await validateTrust({
  ...
  const handleTrust = async () => {
    setTrustedEntities(t => {
      if (t.some((x: Certifier) => x.identityKey === identityKey)) {
        ...
        {fieldsValid && (
          <Box sx={{
            ...
            <Box sx={{
              ...
              <img src={icon} alt={name || 'Provider icon'} style={{ width: '4em', height: '4em', borderRadius: '6px' }} />
              <Box>
