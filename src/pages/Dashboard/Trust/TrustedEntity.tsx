...
interface TrustedEntityProps {
  entity: Certifier
  setTrustedEntities: (updater: (prev: Certifier[]) => Certifier[]) => void
}

const TrustedEntity = ({ entity, setTrustedEntities }: TrustedEntityProps) => {
  ...
  const handleTrustChange = (_: unknown, v: number) => {
    setTrust(v)
  ...
  const formatIdentityKey = (key: string): string => {
    if (!key) return '';
  ...
  const iconSrc = entity.iconUrl || (entity as any).iconURL || deterministicImage(entity.identityKey || entity.name || '')
      ...
      >
        <CardContent sx={{ p: 0 }}>
          ...
          >
            ...
            >
              <Avatar
                src={!iconError ? iconSrc : undefined}
                ...
                sx={{
                  ...
                  border: '1px solid',
                  borderColor: 'divider',
                  bgcolor: iconError ? theme.palette.action.hover : 'transparent',
              ...
              >
                {iconError ? entity.name?.[0]?.toUpperCase() : null}
              </Avatar>
              <Box sx={{ flex: 1 }}>
