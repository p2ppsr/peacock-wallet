...
const Apps: React.FC = () => {
  ...
  useEffect(() => {
    const handleRecentAppsUpdate = (event: CustomEvent) => {
      ...
      if (!activeProfile) {
        const walletManager: any = managers.walletManager
        if (walletManager?.listProfiles) {
          setActiveProfile(walletManager.listProfiles().find((p: any) => p.active))
        }
      }
                  ...
                  <Box sx={{ mb: 4 }}>
                    ...
                    >
                      {pinnedFilteredApps.map((app) => (
                        <Grid2 key={app.domain} size={{ xs: 6, sm: 6, md: 3, lg: 2 }} sx={styles.gridItem}>
