...
const AmountDisplay: React.FC<Props> = ({ abbreviate, showPlus, description, children, showFiatAsInteger }) => {
  ...
  useEffect(() => {
    if (Number.isInteger(Number(children))) {
      ...
      if (description === 'Return to your User Wallet Balance') {
        setFormattedSatoshis(`+${satoshisToDisplay}`)
        setColor('green')
      } else if (description === 'Spend from your User Wallet Balance') {
        setFormattedSatoshis(`-${satoshisToDisplay}`)
  ...
  const renderAccessibleAmount = (content: ReactNode, onClick: () => void) => (
    <Button 
      onClick={onClick}
      variant="text" 
      size="small"
